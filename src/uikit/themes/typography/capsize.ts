...
export type CapsizeOptions = CapHeightWithLineGap | CapHeightWithLeading | FontSizeWithLineGap | FontSizeWithLeading;
...
const normaliseOptions = (options: CapsizeOptions) => {
...
const _computeValues = (options: CapsizeOptions) => {
  ...
  const leadingTrim = (value: number) => value - toScale(specifiedLineHeightOffset);
  ...
  return {
    ...
    capHeightTrim: `${roundTo(leadingTrim(ascentScale - capHeightScale + lineGapScale / 2) * -1, PRECISION)}em`,
    baselineTrim: `${roundTo(leadingTrim(descentScale + lineGapScale / 2) * -1, PRECISION)}em`,
  };
};

export const computeValues = (options: CapsizeOptions): CapsizeValues => {
  return _computeValues(options);
};

export const capsize = (options: CapsizeOptions): CapsizeStyles => {
  return createCss(_computeValues(options));
};

export const createCss = ({ lineHeight, fontSize, capHeightTrim, baselineTrim }: CapsizeValues): CapsizeStyles => {
  return {
    fontSize,
    lineHeight,
    '::before': {
...
export const getCapHeight = ({ fontSize, fontMetrics }: { fontSize: number; fontMetrics: FontMetrics }): number => {
