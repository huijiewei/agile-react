...
const Upload = {
  ...
  upload: (url, option, file, onSuccess, onError) => {
    const request = new Request({
      onSuccess: (response) => {
        const result =
          option.dataType === 'xml' ? new DOMParser().parseFromString(response.data, 'application/xml') : response.data;
    ...
    if (params) {
      for (const key in params) {
        ...
        if (value.toString().indexOf('${filename}') !== -1) {
          let randomFileName = Math.random().toString(36).substring(3, 15) + '.' + file.name.split('.').pop();
